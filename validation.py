...
def evaluate_model(model, val_set, tags):
  ...
  for sen in val_set['text']:
    predictions.append(viterbi(model, sen, tags))

  predictions = flatten_double_lst(predictions)
  indices = flatten_double_lst(val_set['index'])

  correct_labels = flatten_double_lst(val_set['NER'])

  pred_dict = format_output_labels(predictions, indices)
  true_dict = format_output_labels(correct_labels, indices)

  return mean_f1(pred_dict, true_dict)
