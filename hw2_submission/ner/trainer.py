...
class Trainer(object):
    ...
    def _train_epoch(self, dataloader) -> Dict[str, float]:
        ...
        for batch in dataloader:
          ...
          batch_padding_mask = batch['padding_mask']
          ...
          # Get Predictions
          preds_tensor = self.model(batch_ids.to(self.device))

          # Compute Loss
          loss_tensor = compute_loss(self.loss_fn, preds_tensor, batch_labels.to(self.device))
          ...
          # Clip Gradients
          if not self.grad_clip_max_norm == None:
            nn.utils.clip_grad_norm_(self.model.parameters(), self.grad_clip_max_norm)

          # Step the optimizer, as time goes on learning rate will decrease
          self.optimizer.step()

          # Compute Batch Level Metrics
          batch_metrics = compute_metrics(preds_tensor, batch_labels, batch_padding_mask, self.labels_ignore_idx, self.other_ner_tag_idx)

          # Append metrics to the lists of metrics
          metrics['loss'].append(v)
          for (field, score) in batch_metrics.items():
    ...
    def _eval_epoch(self, dataloader) -> Dict[str, float]:
        ...
        for batch in dataloader:
          ...
          # Move all tensors and the actual model onto the specified device
